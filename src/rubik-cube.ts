import { SOLVED_PATTERN } from "./utils/cubePatterns";
...
type ColourShorthand = (typeof FACE_COLOURS)[keyof typeof FACE_COLOURS];
type ColourMap = { [key in ColourShorthand]: string };
type FaceColor = { [key in ColourShorthand]: string[] };
...
export class RubikCube {
  ...
  constructor(colorStr?: string) {
    if (!colorStr) {
      colorStr = SOLVED_PATTERN;
    }
    this.colors = colorStr.trim().split("");

    this.generateCubelets();
    this.generateColors();
  }
  ...
  generateColors() {
    const colorNames = Object.values(FACE_COLOURS);
    ...
    for (const cubelet of this.cubelets) {
      ...
      if (y === 1) {
        const i = num;
        const colour = FACE_COLOURS.white;
        cubeColor[colour] = c[faceColor[colour][i]];
      }
      ...
      if (y === -1) {
        ...
        const colour = FACE_COLOURS.yellow;
        cubeColor[colour] = c[faceColor[colour][i]];
      }
      ...
      if (x === -1) {
        const i = num / 3;
        const colour = FACE_COLOURS.orange;
        cubeColor[colour] = c[faceColor[colour][i]];
      }
      ...
      if (z === 1) {
        ...
        const colour = FACE_COLOURS.green;
        cubeColor[colour] = c[faceColor[colour][i]];
      }
      ...
      if (z === -1) {
        ...
        const i = Math.floor(n / 3) * 3 + (3 - (n % 3)) - 1;
        const colour = FACE_COLOURS.blue;
        cubeColor[colour] = c[faceColor[colour][i]];
      }
